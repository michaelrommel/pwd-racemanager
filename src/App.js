...
class App extends Component {
  ...
  componentDidMount () {
    ...
    const newState = rehydrateStateWithLocalStorage('app', this.state);
    for (const key of this.dontrestore) {
      delete newState[key];
    }
  ...
  mergeHeat = (heat) => {
    ...
    const lanes = this.state.currentHeat;
  ...
  getRacetrackStatus = async () => {
    ...
    const raceId = await getCurrentRace(this.state.urlprefix, this.state.user);
    ...
    const currentHeat = getCurrentHeat(this.state.urlprefix, this.state.user, raceId);
    console.log('App::getRacetrackStatus: getting next heat');
    const nextHeat = getNextHeat(this.state.urlprefix, this.state.user, raceId);
    console.log('App::getRacetrackStatus: getting leaderboard');
    const leaderboard = getLeaderboard(this.state.urlprefix, this.state.user, raceId);
    console.log('App::getRacetrackStatus: getting highscore');
    const highscore = getHighscore(this.state.urlprefix, this.state.user, raceId);
  ...
  updateCurrentNextHeat = async () => {
    ...
    const currentHeat = getCurrentHeat(this.state.urlprefix,
      this.state.user, this.state.raceId);
    console.log('App::updateCurrentNextHeat: getting next heat');
    const nextHeat = getNextHeat(this.state.urlprefix,
      this.state.user, this.state.raceId);
    // assemble the state once all promises returned
    const newState = {
      currentHeat: await currentHeat,
